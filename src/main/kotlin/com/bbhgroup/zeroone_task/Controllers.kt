...
class UserController( val userService: UserService){
    ...
    @GetMapping("get-all")
    fun getAll(@RequestParam(required = false) role: String?,
               @RequestParam(required = false) startTime: String?,
               @RequestParam(required = false) endTime: String?,
               pageable: Pageable) = userService.getAll(role, startTime, endTime, pageable)
...
class SessionController( val sessionService: SessionService){
    ...
    fun getAll(@RequestParam(required = false) startTime: String?,
               @RequestParam(required = false) endTime: String?,
               pageable: Pageable)=sessionService.getAll(startTime, endTime, pageable)
